...
void AsymBin( const char* rootfile ){
  ...
  std::set<Double_t> runlist, runlistc12;
  for( int i = 0; i<nentries; i++){
    tDVCS->GetEntry(i);
    
    if( *TargetType == nh3t || *TargetType == nh3b ){ runlistc12.insert(RunNumber); }

    if( *TargetType == nh3t || *TargetType == nh3b ){// *TargetType == nh3s ){
      runlist.insert(RunNumber);
  ...
  std::cout << " >> NH3 RL " << runlist.size() << std::endl;
  std::cout << " >> C12 RL " << runlistc12.size() << std::endl;
